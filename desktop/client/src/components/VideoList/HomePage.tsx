...
import { Youtube, Upload, Headphones, Loader2, Music } from 'lucide-react';
...
import UploadAudio from '../AddVideo/UploadAudio';
...
export default function HomePage() {
  ...
  const [modal, setModal] = useState<'youtube' | 'upload' | 'audio' | null>(null);
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-6">
        <button
          ...
          className="flex items-center justify-center gap-2 py-3 px-3 bg-red-600/20 active:bg-red-600/50 border border-red-500/40 text-red-400 rounded-2xl text-sm font-medium transition-colors"
        >
          <Youtube className="w-4 h-4 shrink-0" />
          <span className="truncate">{t('add_youtube_short')}</span>
        </button>
        <button
          onClick={() => setModal('upload')}
          className="flex items-center justify-center gap-2 py-3 px-3 bg-blue-600/20 active:bg-blue-600/50 border border-blue-500/40 text-blue-400 rounded-2xl text-sm font-medium transition-colors"
        >
          <Upload className="w-4 h-4 shrink-0" />
          <span className="truncate">{t('upload_video_short')}</span>
        </button>
        <button
          onClick={() => setModal('audio')}
          className="flex items-center justify-center gap-2 py-3 px-3 bg-purple-600/20 active:bg-purple-600/50 border border-purple-500/40 text-purple-400 rounded-2xl text-sm font-medium transition-colors"
        >
          <Music className="w-4 h-4 shrink-0" />
          <span className="truncate">{t('upload_audio_short')}</span>
        </button>
      </div>
      ...
      {modal === 'audio' && <UploadAudio onClose={() => setModal(null)} onAdded={load} />}
    </div>
  );
}
