...
router.get('/', async (req, res) => {
  ...
  const secretFile = '/etc/secrets/yt-cookies.txt'; // Render mounts secret files here automatically
  ...
  async function tryDownload(langCodes, autoSubs) {
    ...
    try {
      await execAsync(
        `"${ytDlpBin}" --skip-download ${flag} --sub-langs "${subLangs}" --sub-format vtt --extractor-args "youtube:player_client=android,web" ${cookiesArg} -o "${tmpBase}" "${url}"`,
        { timeout: 30000 }
      );
  ...
  try {
    await execAsync(
      `"${ytDlpBin}" --skip-download --write-auto-subs --sub-format vtt --extractor-args "youtube:player_client=android,web" ${cookiesArg} -o "${tmpBase}" "${url}"`,
      { timeout: 30000 }
    );
    const vttFile = findVttFile();
    if (vttFile) {
