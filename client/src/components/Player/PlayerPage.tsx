...
import { getVideoById, getExercises, recordAttempt, markCompleted, addXp } from '../../services/db';
...
export default function PlayerPage() {
  ...
  const handleNext = useCallback((wasCorrect?: boolean) => {
    ...
    // If wrong — rewind to start of the phrase and repeat
    if (wasCorrect === false && exercise) {
      setCurrentExercise(null);
      setLastResult(null);
      exerciseIdxRef.current -= 1; // repeat this exercise
      resumeVideo(Math.max(0, exercise.time_start - 1));
      return;
    }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-black aspect-video mb-3">
        ...
        {/* Tap-to-resume hint when playVideo() fails on Android */}
        ...
        {currentExercise && (
          <QuizOverlay
            ...
            onReplay={() => {
              ...
              resumeVideo(Math.max(0, ex.time_start - 1));
            }}
